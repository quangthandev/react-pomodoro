...
function App() {
	...
	const handleToggleOpenSettings = (open) => {
		if (open) {
	...
	const handleChangeMode = (m) => {
		if (status === 'running') {
			const feedback = window.confirm(
				'The timer is running, are you sure you want to switch mode?'
			);
		...
		>
			<Header mute={mute} onToggleSound={handleToggleSound}>
				<SettingsModal
					...
					close={() => dispatch({ type: CLOSE_SETTINGS })}
					intervals={intervals}
			...
			<Main>
				<StatusBar mode={mode} onChangeMode={handleChangeMode} />
				<ProgressDisplay
					interval={interval}
					mode={mode}
					mute={mute}
